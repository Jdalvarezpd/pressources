...
if(!isset($_SESSION['admin'])){
  ...
  if($_SERVER['REQUEST_METHOD'] == 'GET' && !empty($_GET['busqueda'])){
        ...
        $users = $conexion->prepare("SELECT SQL_CALC_FOUND_ROWS * FROM users WHERE (name LIKE '$busqueda' OR last_name LIKE '$busqueda' OR email LIKE '$busqueda') LIMIT $inicio, $postPorPagina");
        $users->execute();
      ...
      $users = $conexion->prepare("SELECT SQL_CALC_FOUND_ROWS * FROM users LIMIT $inicio, $postPorPagina");
      $users->execute();
      $users = $users->fetchAll();
